...
{
    public record ProdutoEntradaDTO(int IDProduto, int IDProdutoTipo, int IDProdutoSituacao, int IDProdutoFabricante, int IDUsuario, int? IDProdutoMedida, string NMProduto, string? DSProduto, ProdutoCor INProdutoCor, string? LKProdutoImagem, string? TXAnotacao, ProdutoTipoDTO? ProdutoTipo, ProdutoSituacaoDTO? ProdutoSituacao, ProdutoFabricanteDTO? ProdutoFabricante, ProdutoMedidaDTO? ProdutoMedida, IFormFile? Arquivo);
}